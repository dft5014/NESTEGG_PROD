...
export default function Portfolio() {
  ...
  const previousPortfolioValue = usePrevious(portfolioValue);
        ...
        if (response.ok) {
          ...
          if (lastLoginDate) {
            ...
            if (lastLoginDate === yesterdayString) {
              ...
              setStreakCount(parseInt(localStorage.getItem("loginStreak") || "0"));
            }
          } else {
            // First login ever
            localStorage.setItem("loginStreak", "1");
...
// Reset form messages when opening modals
useEffect(() => {
  if (isAddAccountModalOpen || isEditAccountModalOpen || isAddPositionModalOpen || 
      isEditPositionModalOpen || isUSSecuritiesModalOpen || isBulkUploadModalOpen) {
    setFormMessage("");
  }
}, [isAddAccountModalOpen, isEditAccountModalOpen, isAddPositionModalOpen, 
    isEditPositionModalOpen, isUSSecuritiesModalOpen, isBulkUploadModalOpen]);
...
// Update the updateMarketPrices function to use the correct endpoint
const updateMarketPrices = async () => {
  ...
  try {
    const response = await fetch(`${apiBaseUrl}/market/update-prices-v2`, {  // Updated endpoint
      method: "POST",
      headers: { Authorization: `Bearer ${token}` }
    });
    ...
    if (response.ok) {
      ...
      setError(`Failed to update prices: ${errorData.detail || "Unknown error"}`);
    }
  } catch (error) {
...
// Update the handleAddSecurity function
const handleAddSecurity = async () => {
  ...
  try {
    ...
    let securityToUse = exactMatch || verifyData.results[0];
    
    // Prepare the position data according to the backend expectations
    const positionData = {
      ticker: securityToUse.ticker.toUpperCase(),
      shares: parseFloat(securityShares),
      price: parseFloat(securityPrice || securityToUse.price || 0),
...
// Handle bulk upload
// Improved bulk upload with better error handling
const handleBulkUpload = async () => {
  ...
  setFormMessage("Processing bulk upload...");
  const token = localStorage.getItem("token");
  ...
  try {
    for (const line of lines) {
      ...
      if (line.includes('\t')) {
        ...
        // Use spaces as last resort
        row = line.split(/\s+/);
      }
      
      // Skip if we don't have at least 5 columns
      if (row.length < 5) {
        console.warn(`Skipping row with insufficient columns: ${line}`);
        errorCount++;
        continue;
      }
      ...
      if (!ticker || isNaN(parseFloat(shares)) || isNaN(parseFloat(price)) || 
        ...
        console.warn(`Skipping row with invalid data: ${line}`);
        errorCount++;
        continue;
      }
      ...
      try {
        ...
        processedPositions.push(positionData);
        ...
        if (response.ok) {
          ...
          const errorData = await response.json();
          console.error(`Failed to add position: ${ticker}`, errorData);
          errorCount++;
        }
      } catch (error) {
        console.error(`Error adding position ${ticker}:`, error);
        errorCount++;
      }
    }
    ...
    setFormMessage(`Error uploading positions: ${error.message}`);
  } finally {
...
const handleUpdatePosition = async (e) => {
  ...
  try {
    // Create a complete position object with all required fields
    const positionData = {
    ...
    if (response.ok) {
      ...
      const errorData = await response.json();
      setFormMessage(`Failed to update position: ${errorData.detail || 'Unknown error'}`);
    }
  } catch (error) {
...
{streakCount > -1 && (
  <div className="flex items-center bg-blue-50 px-3 py-1 rounded-full">
    ...
    <span className="text-sm font-medium text-blue-700">{streakCount} day streak</span>
  </div>
)}

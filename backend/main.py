...
async def get_portfolio_snapshots_raw(
    ...
    try:
        ...
        SELECT 
            ...
            a.account_type
        FROM portfolio_daily_snapshots pds
        LEFT JOIN accounts a ON pds.account_id = a.id
        WHERE 
            pds.user_id = :user_id AND
            pds.snapshot_date BETWEEN :start_date AND :end_date
        ORDER BY pds.snapshot_date DESC, pds.asset_type, pds.identifier
        """
        ...
        for snapshot in snapshots:
            ...
            position_data = {
                ...
                "gain_loss": (float(snapshot["current_value"]) if snapshot["current_value"] else 0) - 
                            (float(snapshot["total_cost_basis"]) if snapshot["total_cost_basis"] else 0),
            ...
            snapshots_by_date[date_str]["total_gain_loss"] += position_data["gain_loss"]
...
class TagCreate(BaseModel):
    ...
    color: str = Field("#6B7280", regex="^#[0-9A-Fa-f]{6}$")
...
class TaskUpdate(BaseModel):
    ...
    energy_level: Optional[TaskEnergyLevel] = None
    estimated_cost: Optional[float] = None
...
class TaskGroup(TaskGroupCreate):
    id: str
    user_id: str
    is_active: bool = True
    created_at: datetime
...
class RecurringTaskCreate(BaseModel):
    ...
    custom_pattern_days: Optional[int] = None
...
async def get_tasks(
    ...
    sort_by: str = Query("due_date", regex="^(due_date|created_at|priority|status)$"),
    ...
    try:
        ...
        if status:
            query += " AND t.status = ANY(:status)"
            values["status"] = status
        
        if priority:
            query += " AND t.priority = ANY(:priority)"
            values["priority"] = priority
        
        if category:
            query += " AND t.category = ANY(:category)"
            values["category"] = category
        
        if is_financial is not None:
            query += " AND t.is_financial = :is_financial"
            values["is_financial"] = is_financial
        
        if is_mobile_friendly is not None:
            query += " AND t.is_mobile_friendly = :is_mobile_friendly"
            values["is_mobile_friendly"] = is_mobile_friendly
        ...
        if due_before:
            query += " AND t.due_date < :due_before"
            values["due_before"] = datetime.fromisoformat(due_before.replace('Z', '+00:00'))
        
        if due_after:
            query += " AND t.due_date > :due_after"
            values["due_after"] = datetime.fromisoformat(due_after.replace('Z', '+00:00'))
        
        if group_id:
            query += """
                AND EXISTS (
                    SELECT 1 FROM task_group_assignments tga
                    WHERE tga.task_id = t.id AND tga.group_id = :group_id
                )
            """
            values["group_id"] = group_id
        
        if energy_level:
            query += " AND t.energy_level = :energy_level"
            values["energy_level"] = energy_level
        ...
        for row in result:
            task = dict(row)
            # Parse JSON arrays
            task["tags"] = [dict(tag) for tag in task.get("tags", [])]
            task["subtasks"] = [dict(subtask) for subtask in task.get("subtasks", [])]
            
            # Format dates
            for date_field in ["due_date", "reminder_date", "completed_at", "created_at", "updated_at"]:
                if task.get(date_field):
                    task[date_field] = task[date_field].isoformat() if hasattr(task[date_field], "isoformat") else task[date_field]
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to fetch tasks: {str(e)}")

@app.post("/api/tasks", status_code=status.HTTP_201_CREATED)
async def create_task(task: TaskCreate, current_user: dict = Depends(get_current_user)):
    """Create a new task with subtasks and tags"""
    try:
        async with database.transaction():
            ...
            INSERT INTO tasks (
                ...
                energy_level, estimated_cost, estimated_duration_minutes, due_date, 
                reminder_date, is_financial, is_mobile_friendly, location, url, notes
            ) VALUES (
                :user_id, :title, :description, :status, :priority, :category,
                :energy_level, :estimated_cost, :estimated_duration_minutes, :due_date,
                :reminder_date, :is_financial, :is_mobile_friendly, :location, :url, :notes
            ...
            task_values = {
                ...
                "due_date": datetime.fromisoformat(task.due_date.replace('Z', '+00:00')) if task.due_date else None,
                "reminder_date": datetime.fromisoformat(task.reminder_date.replace('Z', '+00:00')) if task.reminder_date else None,
                "is_financial": task.is_financial,
                "is_mobile_friendly": task.is_mobile_friendly,
                "location": task.location,
                "url": task.url,
                "notes": task.notes
            }
            
            task_result = await database.fetch_one(query=task_query, values=task_values)
            ...
            if task.tags:
                for tag_name in task.tags:
                    # Check if tag exists
                    tag_query = "SELECT id, name, color FROM tags WHERE user_id = :user_id AND name = :name"
                    tag = await database.fetch_one(query=tag_query, values={"user_id": current_user["id"], "name": tag_name})
                    ...
                    if not tag:
                        ...
                        tag = await database.fetch_one(
                            ...
                            values={"user_id": current_user["id"], "name": tag_name, "color": "#6B7280"}
                        )
                    
                    created_tags.append(dict(tag))
                    
                    # Link tag to task
                    link_query = "INSERT INTO task_tags (task_id, tag_id) VALUES (:task_id, :tag_id)"
                    await database.execute(query=link_query, values={"task_id": task_id, "tag_id": tag["id"]})
            ...
            if task.subtasks:
                for subtask in task.subtasks:
                    subtask_query = """
                    INSERT INTO subtasks (parent_task_id, title, position)
                    VALUES (:parent_task_id, :title, :position)
                    RETURNING *
                    """
                    subtask_result = await database.fetch_one(
                        query=subtask_query,
                        values={
                    ...
                    created_subtasks.append(dict(subtask_result))
            
            # Assign to groups
            if task.group_ids:
                for group_id in task.group_ids:
                    group_query = "INSERT INTO task_group_assignments (task_id, group_id) VALUES (:task_id, :group_id)"
                    await database.execute(query=group_query, values={"task_id": task_id, "group_id": group_id})
            
            # Create dependencies
            if task.depends_on_task_ids:
                for dep_id in task.depends_on_task_ids:
                    dep_query = "INSERT INTO task_dependencies (task_id, depends_on_task_id) VALUES (:task_id, :depends_on_task_id)"
                    await database.execute(query=dep_query, values={"task_id": task_id, "depends_on_task_id": dep_id})
            
            # Log creation in history
            history_query = """
            INSERT INTO task_history (task_id, user_id, action, new_status)
            VALUES (:task_id, :user_id, 'created', :new_status)
            """
            await database.execute(
                query=history_query,
                values={"task_id": task_id, "user_id": current_user["id"], "new_status": task.status}
            )
            
            # Update productivity stats
            await update_productivity_stats_db(current_user["id"], tasks_created=1)
            
            # Prepare response
            response_task = dict(task_result)
            response_task["tags"] = created_tags
            response_task["subtasks"] = created_subtasks
            response_task["groups"] = []
            response_task["dependencies"] = task.depends_on_task_ids or []
            
            # Format dates
            for date_field in ["due_date", "reminder_date", "created_at", "updated_at"]:
                if response_task.get(date_field):
                    response_task[date_field] = response_task[date_field].isoformat()
...
async def update_task(task_id: str, task_update: TaskUpdate, current_user: dict = Depends(get_current_user)):
    ...
    try:
        # Get current task
        current_query = "SELECT * FROM tasks WHERE id = :task_id AND user_id = :user_id"
        current_task = await database.fetch_one(query=current_query, values={"task_id": task_id, "user_id": current_user["id"]})
        ...
        for field, value in task_update.dict(exclude_unset=True).items():
            if field in ["due_date", "reminder_date", "completed_at"] and value is not None:
                update_values[field] = datetime.fromisoformat(value.replace('Z', '+00:00'))
            else:
        ...
        if update_fields:
            ...
            result = await database.fetch_one(query=query, values=update_values)
            
            # Log status change
            if task_update.status and task_update.status != current_task["status"]:
                history_query = """
                INSERT INTO task_history (task_id, user_id, action, old_status, new_status)
                VALUES (:task_id, :user_id, :action, :old_status, :new_status)
                """
                await database.execute(
                    query=history_query,
                    values={
                        "task_id": task_id,
                        "user_id": current_user["id"],
                        "action": "completed" if task_update.status == "completed" else "updated",
                        "old_status": current_task["status"],
                        "new_status": task_update.status
                    }
                )
                
                # Update productivity stats if completed
                if task_update.status == "completed":
                    await update_productivity_stats_db(
                        current_user["id"],
                        tasks_completed=1,
                        duration_minutes=task_update.actual_duration_minutes or 0,
                        cost=task_update.actual_cost or 0
                    )
            
            # Fetch complete task with relations
            return await get_task_with_relations_db(task_id, current_user["id"])
...
async def delete_task(task_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        task = await database.fetch_one(query=check_query, values={"task_id": task_id, "user_id": current_user["id"]})
...
async def get_task_summary(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        SELECT 
            ...
            COUNT(*) FILTER (WHERE is_financial = TRUE) as financial_count,
            COUNT(*) FILTER (WHERE is_mobile_friendly = TRUE) as mobile_count,
            SUM(actual_cost) as total_spent,
            SUM(estimated_cost) FILTER (WHERE status != 'completed') as budget_remaining,
            SUM(actual_duration_minutes) as total_time_spent
        FROM tasks
        WHERE user_id = :user_id
        """
        counts = await database.fetch_one(query=counts_query, values={"user_id": current_user["id"]})
        ...
        GROUP BY category
        """
        category_stats = await database.fetch_all(query=category_query, values={"user_id": current_user["id"]})
        ...
        GROUP BY priority
        """
        priority_stats = await database.fetch_all(query=priority_query, values={"user_id": current_user["id"]})
        ...
        ORDER BY date DESC
        """
        recent_activity = await database.fetch_all(query=activity_query, values={"user_id": current_user["id"]})
        
        # Upcoming tasks
        upcoming_query = """
        SELECT id, title, due_date, priority, category, estimated_cost
        FROM tasks
        WHERE user_id = :user_id 
        ...
        LIMIT 10
        """
        upcoming_tasks = await database.fetch_all(query=upcoming_query, values={"user_id": current_user["id"]})
        
        # Calculate insights
        counts_dict = dict(counts)
        total_tasks = (counts_dict["completed_count"] or 0) + (counts_dict["pending_count"] or 0) + (counts_dict["in_progress_count"] or 0)
        completion_rate = (counts_dict["completed_count"] or 0) / max(total_tasks, 1)
        
        # Find most productive category
        most_productive_category = None
        if category_stats:
            most_productive = max(category_stats, key=lambda x: x["completed"] / max(x["total"], 1))
            most_productive_category = most_productive["category"]
        ...
        return {
            ...
            "insights": {
                ...
                "avg_daily_completions": sum(r["completed"] for r in recent_activity) / max(len(recent_activity), 1) if recent_activity else 0,
                "most_productive_category": most_productive_category,
                "total_time_hours": (counts_dict["total_time_spent"] or 0) / 60,
                "financial_impact": counts_dict["total_spent"] or 0
            }
        }
        
    except Exception as e:
        logger.error(f"Error getting task summary: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get task summary: {str(e)}")

@app.get("/api/tasks/suggestions")
async def get_task_suggestions(
    context: str = Query("now", regex="^(now|morning|evening|weekend|low_energy|high_focus|financial|mobile)$"),
    ...
    try:
        ...
        LEFT JOIN task_tags tt ON t.id = tt.task_id
        LEFT JOIN tags tg ON tt.tag_id = tg.id
        WHERE t.user_id = :user_id AND t.status IN ('pending', 'in_progress')
        """
        
        values = {"user_id": current_user["id"], "limit": limit}
        
        # Apply context-specific filters
        if context == "now":
            query = base_query + """
                AND (
                    (t.due_date IS NOT NULL AND t.due_date < NOW() + INTERVAL '24 hours')
                    OR t.priority IN ('critical', 'high')
            ...
            query = base_query + """
                AND t.energy_level IN ('high', 'medium')
                AND t.category IN ('work', 'learning', 'health')
                GROUP BY t.id
                ORDER BY t.priority ASC, hours_until_due ASC
            """
        elif context == "evening":
            query = base_query + """
                AND t.energy_level IN ('low', 'minimal')
                AND t.category IN ('personal', 'family', 'home')
                GROUP BY t.id
                ORDER BY t.priority ASC, hours_until_due ASC
            """
            ...
            query = base_query + """
                AND t.category != 'work'
                AND (t.estimated_duration_minutes IS NULL OR t.estimated_duration_minutes > 30)
                GROUP BY t.id
                ORDER BY t.priority ASC, hours_until_due ASC
            """
            ...
            query = base_query + """
                ...
                AND (t.estimated_duration_minutes IS NULL OR t.estimated_duration_minutes <= 30)
                GROUP BY t.id
                ORDER BY t.estimated_duration_minutes ASC NULLS LAST, t.priority ASC
            """
        elif context == "high_focus":
            query = base_query + """
                AND t.energy_level = 'high'
                AND (t.estimated_duration_minutes IS NULL OR t.estimated_duration_minutes >= 60)
                AND t.category IN ('work', 'learning')
                GROUP BY t.id
                ORDER BY t.priority ASC, hours_until_due ASC
            """
        elif context == "financial":
            query = base_query + """
                AND (t.is_financial = TRUE OR t.category IN ('financial', 'bills', 'investments'))
                GROUP BY t.id
                ORDER BY hours_until_due ASC, t.priority ASC
            """
        elif context == "mobile":
            query = base_query + """
                AND t.is_mobile_friendly = TRUE
                GROUP BY t.id
                ORDER BY t.priority ASC, hours_until_due ASC
            """
        ...
        return {
            ...
            "reasoning": get_context_reasoning(context)
        }
        
    except Exception as e:
        logger.error(f"Error getting task suggestions: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get task suggestions: {str(e)}")

@app.get("/api/tasks/stats/productivity")
async def get_productivity_stats(
    ...
    try:
        ...
        # Calculate streak
        streak = await calculate_streak_db(current_user["id"])
        ...
        for stat in stats:
            ...
            GROUP BY category
            """
            category_data = await database.fetch_all(
                query=category_query,
                values={"user_id": current_user["id"], "date": stat["date"]}
            )
            
            category_breakdown = {row["category"]: row["count"] for row in category_data}
            
            stat_dict = dict(stat)
            stat_dict["streak_days"] = streak
            stat_dict["category_breakdown"] = category_breakdown
            stat_dict["completion_rate"] = stat_dict["tasks_completed"] / max(stat_dict["tasks_created"], 1)
        ...
        logger.error(f"Error getting productivity stats: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get productivity stats: {str(e)}")

@app.post("/api/tasks/groups", status_code=status.HTTP_201_CREATED)
async def create_task_group(group: TaskGroupCreate, current_user: dict = Depends(get_current_user)):
    """Create a new task group"""
    try:
        query = """
        INSERT INTO task_groups (user_id, name, description, color, icon, target_completion_date, budget)
        VALUES (:user_id, :name, :description, :color, :icon, :target_completion_date, :budget)
        RETURNING *
        """
        ...
        values = {
            ...
            "color": group.color,
            "icon": group.icon,
            "target_completion_date": datetime.fromisoformat(group.target_completion_date.replace('Z', '+00:00')) if group.target_completion_date else None,
            "budget": group.budget
        }
        
        result = await database.fetch_one(query=query, values=values)
        
        # Get initial stats (will be empty for new group)
        return {
            **dict(result),
            "task_count": 0,
            "completed_count": 0,
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to create task group: {str(e)}")
...
async def get_task_groups(
    ...
    """Get all task groups with statistics"""
    try:
        query = """
        SELECT 
            tg.*,
        ...
        FROM task_groups tg
        LEFT JOIN task_group_assignments tga ON tg.id = tga.group_id
        LEFT JOIN tasks t ON tga.task_id = t.id
        WHERE tg.user_id = :user_id
        """
        
        if not include_inactive:
            query += " AND tg.is_active = TRUE"
        
        query += " GROUP BY tg.id ORDER BY tg.created_at DESC"
        
        groups = await database.fetch_all(query=query, values={"user_id": current_user["id"]})
        
        return [dict(group) for group in groups]
        
    except Exception as e:
        logger.error(f"Error getting task groups: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get task groups: {str(e)}")

# Helper functions
async def get_task_with_relations_db(task_id: str, user_id: str):
    """Fetch a task with all its relations"""
    query = """
    SELECT t.*, 
        COALESCE(
            ARRAY_AGG(
        ...
        COALESCE(
            ARRAY_AGG(DISTINCT td.depends_on_task_id) FILTER (WHERE td.depends_on_task_id IS NOT NULL),
            '{}'
        ) as dependencies
    FROM tasks t
    LEFT JOIN task_tags tt ON t.id = tt.task_id
    LEFT JOIN tags tg ON tt.tag_id = tg.id
    LEFT JOIN subtasks st ON t.id = st.parent_task_id
    LEFT JOIN task_group_assignments tga ON t.id = tga.task_id
    LEFT JOIN task_groups grp ON tga.group_id = grp.id
    LEFT JOIN task_dependencies td ON t.id = td.task_id
    WHERE t.id = :task_id AND t.user_id = :user_id
    GROUP BY t.id
    """
    
    result = await database.fetch_one(query=query, values={"task_id": task_id, "user_id": user_id})
    ...
    task["tags"] = [dict(tag) for tag in task.get("tags", [])]
    task["subtasks"] = [dict(subtask) for subtask in task.get("subtasks", [])]
    task["groups"] = [dict(group) for group in task.get("groups", [])]
    task["dependencies"] = list(task.get("dependencies", []))
...
async def update_productivity_stats_db(user_id: str, tasks_completed: int = 0, 
                                     tasks_created: int = 0, duration_minutes: int = 0, 
                                     cost: float = 0):
    ...
    check_query = "SELECT * FROM productivity_stats WHERE user_id = :user_id AND date = :date"
    existing = await database.fetch_one(query=check_query, values={"user_id": user_id, "date": today})
    
    if existing:
        # Update existing
        new_completed = existing["tasks_completed"] + tasks_completed
        new_created = existing["tasks_created"] + tasks_created
        new_duration = existing["total_duration_minutes"] + duration_minutes
        new_cost = float(existing["total_cost_tracked"]) + cost
        
        # Calculate focus score
        focus_score = calculate_productivity_score(new_completed, new_created, new_duration)
        
        # Update most productive hour if task was completed
        most_productive = existing["most_productive_hour"]
        if tasks_completed > 0:
            most_productive = current_hour
        
        update_query = """
        UPDATE productivity_stats
        SET tasks_completed = :tasks_completed, tasks_created = :tasks_created, 
            total_duration_minutes = :total_duration_minutes, total_cost_tracked = :total_cost_tracked,
            focus_score = :focus_score, most_productive_hour = :most_productive_hour
        ...
        await database.execute(
            ...
            values={
                ...
                "tasks_created": new_created,
                "total_duration_minutes": new_duration,
                "total_cost_tracked": new_cost,
        ...
        # Create new
        focus_score = calculate_productivity_score(tasks_completed, tasks_created, duration_minutes)
        
        create_query = """
        INSERT INTO productivity_stats (
            user_id, date, tasks_completed, tasks_created, total_duration_minutes,
            total_cost_tracked, focus_score, most_productive_hour
        ) VALUES (
            :user_id, :date, :tasks_completed, :tasks_created, :total_duration_minutes,
            :total_cost_tracked, :focus_score, :most_productive_hour
        )
        ...
        await database.execute(
            ...
            values={
                ...
                "total_cost_tracked": cost,
                "focus_score": focus_score,
                "most_productive_hour": current_hour if tasks_completed > 0 else None
            }
        )

def calculate_productivity_score(completed: int, created: int, duration_minutes: int) -> int:
    """Calculate productivity score (0-100)"""
    if created == 0:
        completion_rate = 1.0
    else:
        completion_rate = min(completed / created, 1.0)
    
    # Base score from completion rate (0-70 points)
    score = int(completion_rate * 70)
    
    # Bonus for focus time (0-30 points)
...
async def calculate_streak_db(user_id: str) -> int:
    ...
    return result["streak_days"] if result and result["streak_days"] else 0

def get_context_reasoning(context: str) -> str:
    """Get explanation for context-based suggestions"""
    reasoning = {
        "now": "Showing urgent tasks, in-progress work, and high-priority items that need immediate attention",
        "morning": "High-energy tasks perfect for when you're fresh and focused",
        "evening": "Low-energy personal tasks ideal for winding down",
        "weekend": "Non-work tasks to help you relax and take care of personal matters",
        "low_energy": "Quick, easy tasks you can knock out when you're not at peak performance",
        "high_focus": "Deep work requiring sustained concentration and energy",
        "financial": "Money-related tasks to keep your finances on track",
        "mobile": "Tasks you can complete on your phone while commuting or waiting"
    }
    return reasoning.get(context, "Personalized task suggestions")
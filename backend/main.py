...
from fastapi import FastAPI, Depends, HTTPException, status, Query, File
...
async def get_categories():
    ...
    try:
        ...
        return [Category(**dict(row)) for row in result]
    except Exception as e:
...
async def get_tasks(
    ...
    """Get filtered tasks for the current user"""
    try:
        ...
        if status:
            ...
            values["status"] = [s.value for s in status]
        
        if priority:
            query += " AND t.priority = ANY(:priority)"
            values["priority"] = [p.value for p in priority]
        ...
        if tags:
            query += " AND t.tags && :tags"  # && is the array overlap operator
            values["tags"] = tags
        ...
        # Sort and paginate
        query += f" ORDER BY t.{sort_by} ASC NULLS LAST"
        ...
        for row in result:
            ...
            # Build category object if exists
            if task_dict.get("category_id"):
                task_dict["category"] = {
                    "id": task_dict["category_id"],
                    "name": task_dict["category_name"],
                    "icon": task_dict["category_icon"],
                    "color": task_dict["category_color"],
                    "is_system": task_dict["category_is_system"]
                }
            
            # Clean up extra fields
            for field in ["category_name", "category_icon", "category_color", "category_is_system"]:
...
async def create_task(task: TaskCreate, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        task_query = """
        INSERT INTO tasks (
            user_id, title, description, status, priority, category_id, 
            energy_level, estimated_cost, estimated_duration_minutes, 
            due_date, is_financial, is_mobile_friendly, tags
        ) VALUES (
            :user_id, :title, :description, :status, :priority, :category_id,
            :energy_level, :estimated_cost, :estimated_duration_minutes,
            :due_date, :is_financial, :is_mobile_friendly, :tags
        ...
        task_values = {
            ...
            "tags": task.tags  # PostgreSQL array
        }
        
        task_result = await database.fetch_one(query=task_query, values=task_values)
        
        # Fetch with category
        return await get_task_with_category(task_result["id"], current_user["id"])
...
async def update_task(task_id: str, task_update: TaskUpdate, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        check_query = "SELECT * FROM tasks WHERE id = :task_id AND user_id = :user_id"
        existing_task = await database.fetch_one(query=check_query, values={"task_id": task_id, "user_id": current_user["id"]})
        ...
        for field, value in task_update.dict(exclude_unset=True).items():
            if value is not None:
                if field in ["due_date", "completed_at"] and isinstance(value, str):
                    update_values[field] = datetime.fromisoformat(value.replace('Z', '+00:00'))
                elif field in ["status", "priority", "energy_level"] and hasattr(value, 'value'):
        ...
        if update_fields:
            ...
            await database.fetch_one(query=query, values=update_values)
        
        # Fetch updated task with category
        return await get_task_with_category(task_id, current_user["id"])
...
async def get_task_stats(current_user: dict = Depends(get_current_user)):
    """Get task statistics calculated on the fly"""
    try:
        # Get all tasks with categories for stats calculation
        tasks_query = """
        ...
        WHERE t.user_id = :user_id
        """
        tasks = await database.fetch_all(query=tasks_query, values={"user_id": current_user["id"]})
        ...
        overdue_count = sum(1 for t in tasks if t["due_date"] and t["due_date"] < datetime.now() and t["status"] != "completed")
        
        completion_rate = completed_count / total_tasks if total_tasks > 0 else 0
        
        total_spent = sum(t["actual_cost"] or 0 for t in tasks)
        total_estimated = sum(t["estimated_cost"] or 0 for t in tasks if t["status"] != "completed")
        total_time_minutes = sum(t["actual_duration_minutes"] or 0 for t in tasks)
        total_time_hours = total_time_minutes / 60
        
        # Category breakdown
        category_breakdown = {}
        for task in tasks:
            cat = task["category_name"] or "Uncategorized"
            ...
            category_breakdown[cat]["total_cost"] += task["actual_cost"] or 0
            category_breakdown[cat]["total_time"] += task["actual_duration_minutes"] or 0
        ...
        LIMIT 10
        """
        upcoming_result = await database.fetch_all(query=upcoming_query, values={"user_id": current_user["id"]})
        ...
        for row in upcoming_result:
            ...
            for field in ["category_name", "category_icon", "category_color", "category_is_system"]:
                task_dict.pop(field, None)
            upcoming_tasks.append(Task(**task_dict))
...
@app.get("/api/tasks/suggestions", response_model=List[Task])
async def get_task_suggestions(
    ...
    try:
        ...
        if context == "now":
            query = base_query + """
                AND (
                    ...
                    OR (t.due_date IS NOT NULL AND t.due_date < NOW() + INTERVAL '24 hours')
                )
                ORDER BY 
            ...
            query = base_query + """
                AND t.energy_level IN ('low', 'minimal')
                ORDER BY t.estimated_duration_minutes ASC, t.priority ASC
            """
        elif context == "high_focus":
            query = base_query + """
                AND t.energy_level = 'high'
                AND c.name IN ('Work', 'Learning')
                ORDER BY t.priority ASC, t.due_date ASC NULLS LAST
            """
        elif context == "financial":
            query = base_query + """
                AND (t.is_financial = TRUE OR c.name IN ('Financial', 'Bills', 'Investments'))
        ...
        for row in result:
            ...
            for field in ["category_name", "category_icon", "category_color", "category_is_system"]:
                task_dict.pop(field, None)
            suggestions.append(Task(**task_dict))
...
# Helper function to get task with category
async def get_task_with_category(task_id: str, user_id: str):
    """Fetch a task with its category"""
    query = """
    SELECT t.*,
        c.id as category_id,
        c.name as category_name,

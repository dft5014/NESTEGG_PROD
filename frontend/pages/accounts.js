...
export default function AccountsPage() {
    ...
    const institutionColors = useMemo(() => ({
        'Vanguard': { start: '#ef4444', end: '#db2777' },
        'Fidelity': { start: '#22c55e', end: '#0d9488' },
        'Charles Schwab': { start: '#3b82f6', end: '#4f46e5' },
        'Robinhood': { start: '#34d399', end: '#16a34a' },
        'Chase': { start: '#2563eb', end: '#1e40af' },
        'E*TRADE': { start: '#a855f7', end: '#4f46e5' },
        'Other': { start: '#6b7280', end: '#374151' }
    }), []);
    ...
    const calculateAccountMetrics = useCallback((accountsData, portfolioInfo) => {
        ...
        const institutionBreakdown = Object.values(institutionMap).map(inst => ({
            ...
            colors: institutionColors[inst.name] || institutionColors.Other
    ...
    }, [institutionGradients, institutionColors]);
                                ...
                                <defs>
                                    {accountsMetrics.institutionBreakdown?.map((inst, index) => (
                                        <linearGradient key={`grad-${inst.name}`} id={`grad-${index}`} x1="0%" y1="0%" x2="100%" y2="100%">
                                            <stop offset="0%" stopColor={inst.colors.start} />
                                            <stop offset="100%" stopColor={inst.colors.end} />
                                        </linearGradient>
                                    ))}
                                </defs>

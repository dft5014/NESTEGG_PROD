    ...
    <div className="fixed inset-0 z-[100]">
      ...
      <div className="absolute inset-4 md:inset-10 rounded-2xl bg-gray-950 border border-gray-800 shadow-2xl flex flex-col">
        <div className="px-5 py-3 border-b border-gray-800 flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-800"><X className="h-5 w-5" /></button>
        </div>
...
// Optional sparkline (used in KPIs/hover tooltips)
...
export default function Portfolio() {
  ...
  const byClassTS = useMemo(() => {
    // expected shape if available: assetPerformance.timeseries_by_class[key] = [{date, pl | value}, ...]
  ...
  const netWorthMixData = useMemo(() => {
    if (!summary) return [];
    return [
      { key:'securities', name:'Securities', value: summary.assetAllocation.securities.value, pct: summary.netWorthMix.securities*100, color: assetColors.securities },
      { key:'netCash', name:'Net Cash', value: summary.altNetWorth.netCash, pct: summary.netWorthMix.netCash*100, color: assetColors.cash },
      { key:'crypto', name:'Crypto', value: summary.assetAllocation.crypto.value, pct: summary.netWorthMix.crypto*100, color: assetColors.crypto },
      { key:'metals', name:'Metals', value: summary.assetAllocation.metals.value, pct: summary.netWorthMix.metals*100, color: assetColors.metals },
      { key:'realEstate', name:'Real Estate', value: summary.altNetWorth.realEstate, pct: summary.netWorthMix.realEstateEquity*100, color: assetColors.real_estate },
      { key:'other', name:'Other Assets', value: summary.altNetWorth.netOtherAssets, pct: summary.netWorthMix.netOtherAssets*100, color: assetColors.other },
  ...
  // Helpers: range slicer for KPI sparklines
  const rangeSlice = (id) => {
    if (!chartData.length) return [];
    const mapDays = { '1w':7, '1m':30, '3m':90, '6m':180, '1y':365 };
    if (id==='ytd') return chartData; // simple fallback
    if (id==='all') return chartData;
  ...
  const AllocationTooltip = ({ active, payload }) => {
    ...
    // Detailed breakdown (Assets/Liabilities/Net) on hover
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-400">Last updated: {lastFetched ? new Date(lastFetched).toLocaleString() : '—'}</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="hidden sm:block"><TimeframeSelector selected={timeframe} onChange={setTimeframe} /></div>
            <button onClick={refreshData} className={`inline-flex items-center gap-2 rounded-xl px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 transition ${isLoading?'opacity-70 cursor-not-allowed':''}`} disabled={isLoading}>
        ...
        {/* NEW: Performance Over Time KPIs (full-width rail with sparklines) */}
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-8">
          {[
            ...
            const sliced = id==='1d' ? chartData.slice(-2) : rangeSlice(id.replace('d','m')); // simple mini range
            return (
              <motion.div key={id} whileHover={{y:-2}} className="bg-gray-900/70 border border-gray-800 rounded-2xl p-3 flex items-center justify-between">
                ...
                <Sparkline data={sliced} dataKey="value" />
              </motion.div>
            );
          })}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-4 space-y-6">
            {/* NET WORTH MIX with detailed hover */}
            <Section
              title="Net Worth Mix"
              ...
              <div className="mt-3 space-y-2">
                {netWorthMixData.map((row,i)=>{
                  const detail = computeMixBreakdown(summary, row.key);
                  return (
                    <div key={i} className="group flex items-center justify-between text-sm px-2 py-2 rounded hover:bg-gray-800/50">
                      ...
                      <div className="flex items-center gap-4">
                        <span className="text-indigo-300 w-12 text-right">{row.pct.toFixed(1)}%</span>
                        <span className="text-white w-28 text-right">{formatCurrency(row.value, showInThousands)}</span>
                      </div>

                      {/* hover detail chip */}
                      {detail && (
                        <div className="absolute opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 transition pointer-events-none">
                          <div className="mt-10 -mr-2 p-3 bg-gray-900 border border-gray-800 rounded-lg shadow-xl text-xs">
                            <div className="flex items-center justify-between gap-6"><span className="text-gray-400">Assets</span><span className="text-gray-100">{formatCurrency(detail.assets, showInThousands)}</span></div>
                            <div className="flex items-center justify-between gap-6"><span className="text-gray-400">Liabilities</span><span className="text-gray-100">{formatCurrency(detail.liabilities, showInThousands)}</span></div>
                            <div className="flex items-center justify-between gap-6 pt-1 border-t border-gray-800 mt-1"><span className="text-gray-300 font-medium">Net</span><span className="text-indigo-300 font-semibold">{formatCurrency(row.value, showInThousands)}</span></div>
                          </div>
                        </div>
                      )}
                    </div>
            ...
            <Section title="Invested Amount" icon={<BarChart3 className="h-5 w-5 text-indigo-300" />}>
              ...
              {[
                ...
                <InvestedRow
                  ...
                  onClick={()=>{
                    if (byClassTS[r.key]) { setModalAssetKey(r.key); setModalOpen(true); }
                  }}
                />
              ))}
            ...
            {netCashBasisMetrics && (
              <Section
                ...
                right={
                  <div className="flex items-center gap-2">
                    <button onMouseEnter={()=>{}} className="text-xs px-2 py-1 rounded-lg bg-gray-800 text-gray-300 cursor-default">
              ...
              >
                <div className="text-xs text-gray-400">
                  Hover tip: values update daily. Click “Open” for the full chart.
                </div>
              </Section>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            {[
              ...
              <div key={label} className="bg-gray-900/70 border border-gray-800 rounded-xl p-3">
                ...
                <p className={`text-[11px] ${pct>0?'text-emerald-400':pct<0?'text-rose-400':'text-gray-500'}`}>{pct?formatPct(pct*100):'—'}</p>
              </div>
            ))}
          </div>
        </div>
...
function InvestedRow({ key:rowKey, label, color, market=0, cost=0, gain=null, gainPct=null, showInThousands, sparkData=null, onClick }) {
  ...
  return (
    <div
      className={`relative grid grid-cols-12 px-2 py-2 rounded-lg hover:bg-gray-800/50 ${sparkData?'cursor-pointer':''}`}
...
function computeMixBreakdown(summary, key) {
  ...
  switch (key) {
    ...
    case 'other':
      // other = (otherAssets assets minus real estate assets already counted) – other liabilities

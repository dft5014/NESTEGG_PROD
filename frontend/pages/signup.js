...
// Post-signup: optional plan picker (can be triggered later from Portfolio)
...
function PageShell({ clerkComplete }) {
  ...
  // If redirected after Clerk sign-up, immediately exchange token then go to /portfolio
  useEffect(() => {
    const run = async () => {
      if (!clerkComplete || !isLoaded || !isSignedIn || exchangingRef.current) return;
    ...
    run();
  }, [clerkComplete, isLoaded, isSignedIn, getToken, router]);

  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
      <SignupContent />
      {/* Optional: keep plan picker available (not blocking sign-up success) */}
      {/* You can show this later from /portfolio if desired */}

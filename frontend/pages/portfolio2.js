...
export default function PositionsPage() {
  ...
  const isRefreshing = false; // Will be handled by DataStore
  ...
  // Derive position metrics from DataStore
  const positionMetrics = useMemo(() => {
    const periodChange = getPeriodData ? getPeriodData(selectedTimeframe) : null;
    
    return {
      totalValue: portfolioData?.totalAssets || 0,
      totalGainLoss: portfolioData?.unrealizedGain || 0,
      totalGainLossPercent: portfolioData?.unrealizedGainPercent || 0,
      largestPosition: topPositions?.[0] ? {
        name: topPositions[0].name,
        value: topPositions[0].current_value
      } : null,
      mostProfitablePosition: topPerformersPercent?.[0] ? {
        name: topPerformersPercent[0].name,
        gainLossPercent: topPerformersPercent[0].gain_loss_percent
      } : null,
      periodChanges: {
        '1d': periodChange?.percentChange || 0,
        '1w': trends?.week?.percentChange || 0,
        '1m': trends?.month?.percentChange || 0,
        'ytd': trends?.ytd?.percentChange || 0
      },
      performanceData: trends?.chartData || []
    };
  }, [portfolioData, topPositions, topPerformersPercent, trends, selectedTimeframe, getPeriodData]);
              ...
              <div className="lg:col-span-2">
                ...
                <div className="flex items-center space-x-3">
                  <div className={`flex items-center ${
                    (trends?.[selectedTimeframe]?.percentChange || 0) >= 0 ? 'text-green-400' : 'text-red-400'
                  }`}>
                    {(trends?.[selectedTimeframe]?.percentChange || 0) >= 0 ? (
                      <TrendingUp className="w-5 h-5 mr-1" />
                    ) : (
                      <TrendingDown className="w-5 h-5 mr-1" />
                    )}
                    <span className="text-lg font-medium">
                      {formatPercentage(trends?.[selectedTimeframe]?.percentChange || 0)}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className={`text-lg font-medium ${
                      (trends?.[selectedTimeframe]?.valueChange || 0) >= 0 ? 'text-green-400' : 'text-red-400'
                    }`}>
                      {showValues ? formatCurrency(trends?.[selectedTimeframe]?.valueChange || 0) : '••••'}
                    </span>
                  </div>
                </div>

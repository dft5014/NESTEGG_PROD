...
const UnifiedGroupPositionsTable2 = ({ 
 ...
 const formatPerformanceChange = (percentValue, dollarValue) => {
   if (percentValue === 100 && dollarValue > 0) {
   ...
   const colorClass = isPositive ? 'text-green-400' : percentValue < 0 ? 'text-red-400' : 'text-gray-400';
   
   return (
     <div className="group relative">
       <span className={colorClass}>
         {percentValue !== 0 && (isPositive ? '+' : '')}{percentValue.toFixed(2)}%
       </span>
       {dollarValue !== null && dollarValue !== undefined && (
         <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-1 px-2 py-1 bg-gray-900 text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">
           {isPositive && '+'}{formatCurrency(dollarValue)}
         </div>
       )}
     </div>
   );
         ...
         <div className="overflow-x-auto">
           <table className="min-w-full divide-y divide-gray-700">
             ...
             <tbody className="bg-gray-800/30 divide-y divide-gray-700">
               ...
               {/* Liquid Assets Row */}
               <tr className="bg-green-900/20 font-medium border-b-2 border-green-700">
                 <td className="px-3 py-3 text-center">
                   <span className="font-bold text-green-400">◆</span>
                 </td>
                 <td className="px-3 py-3 whitespace-nowrap">
                   <div className="font-medium text-green-400">Liquid Assets</div>
                   <div className="text-xs text-gray-400">{liquidSummary.count} positions</div>
                 </td>
                 <td className="px-3 py-3"></td>
                 <td className="px-3 py-3"></td>
                 ...
                 <td className="px-3 py-3 text-right">
                   <div className={`font-medium ${liquidSummary.totalGainLoss >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                     {liquidSummary.totalGainLoss >= 0 ? '+' : ''}{formatCurrency(liquidSummary.totalGainLoss)}
                   </div>
                   <div className={`text-xs ${liquidSummary.totalGainLoss >= 0 ? 'text-green-400' : 'text-red-400'}`}>
               ...
               {processedPositions.map((position, index) => (
                 ...
                 >
                   ...
                   <td className="px-3 py-3 whitespace-nowrap text-sm text-right">
                     {position.asset_type !== 'cash' && position.asset_type !== 'real_estate' && position.asset_type !== 'other_asset' ? (
                       <div>
                   ...
                   <td className="px-3 py-3 whitespace-nowrap text-sm text-right text-gray-400">
                     ...
                     {position.asset_type !== 'cash' && position.asset_type !== 'real_estate' && position.asset_type !== 'other_asset' && position.weighted_avg_cost > 0 && (

...
const TUTORIAL_STEPS = [
  {
    title: "Welcome to Smart Reconciliation",
    description: "Keep your NestEgg accurate with our intuitive reconciliation workflow",
    icon: Target
  },
  {
    title: "Quick Updates",
    description: "Start with cash and credit cards - these change most frequently",
    icon: Zap
  },
  {
    title: "Account Validation",
    description: "Verify total account balances match your statements",
    icon: CheckCircle2
  },
  {
    title: "Position Details",
    description: "Drill down to update individual positions when needed",
    icon: BarChart3
  },
  {
    title: "Track Progress",
    description: "Earn achievements and maintain streaks to stay motivated",
    icon: Trophy
  }
];
...
const TutorialModal = ({ isOpen, onClose, currentStep, onNext, onPrev, onComplete }) => {
          ...
          >
              ...
              {TUTORIAL_STEPS.map((_, index) => (
                <div
                  ...
                  className={`h-2 rounded-full transition-all ${
                    ...
                    index < currentStep ? 'w-2 bg-blue-400' : 'w-2 bg-gray-300'
                  }`}
                />
              ))}
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              {currentStep < TUTORIAL_STEPS.length - 1 ? (
                ...
                <button
                  onClick={onComplete}
                  className="px-6 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl hover:from-blue-700 hover:to-purple-700 font-medium"
                >
                  Get Started
                </button>
              )}
...
const QuickReconciliationModal2 = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const hasSeenTutorial = localStorage.getItem('nestegg_tutorial_seen');
      if (!hasSeenTutorial) {
  ...
  const processedAccounts = useMemo(() => {
    ...
    return dataStoreAccounts.map(account => {
      ...
      const totalValue = accountPositions.reduce((sum, p) => sum + (p.current_value || 0), 0);
  ...
  const handleAccountReconciliation = (accountId, statementBalance) => {
    ...
    showMessage('success', `Account ${account.name} reconciled`);
  };
  ...
  const handleTutorialComplete = () => {
    ...
    localStorage.setItem('nestegg_tutorial_seen', 'true');
  };
  ...
  // RENDER METHODS - NOW WITH ACCESS TO ALL STATE
  const renderDashboard = () => (
    <div className="space-y-6">
      ...
      {/* Recent Activity & Portfolio Breakdown */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        >
          ...
          {reconciliationHistoryRef.current.length > 0 ? (
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{formatTimeAgo(rec.date)}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-medium text-gray-900">{rec.health}%</p>
                    {rec.timeTaken && rec.timeTaken < 120000 && (
                      <p className="text-xs text-green-600">Quick! 🚀</p>
                    )}
                  </div>
                </motion.div>
              ))}
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-400 mt-1">Start your first reconciliation to build history</p>
            </div>
          )}
        </motion.div>
        ...
        >
          ...
          <div className="space-y-4">
            {Object.entries(
              processedPositions.reduce((acc, pos) => {
                const type = ASSET_CONFIGS[pos.asset_type]?.label || 'Other';
                acc[type] = (acc[type] || 0) + 1;
                return acc;
              }, {})
            ).map(([type, count]) => {
              const percentage = processedPositions.length > 0 ? 
                (count / processedPositions.length) * 100 : 0;
              const config = Object.values(ASSET_CONFIGS).find(c => c.label === type);
              
              return (
                <div key={type}>
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center space-x-2">
                      {config && <config.icon className={`w-4 h-4 ${config.textColor}`} />}
                      <span className="text-sm font-medium text-gray-700">{type}</span>
                    </div>
                    <span className="text-sm text-gray-500">{count} positions</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <motion.div
                      ...
                      className={`h-2 rounded-full bg-gradient-to-r ${config?.gradient || 'from-gray-400 to-gray-500'}`}
                    />
                  </div>
                </div>
              );
                 ...
                 <div className="p-5">
                   ...
                   {!variance && (
                     <div className="flex items-center space-x-3">
                       ...
                       <button
                         onClick={() => {
                           const input = document.getElementById(`reconcile-input-${account.id}`);
             ...
             <div className="flex items-center space-x-3">
               ...
               >
                 <RefreshCw className={`w-5 h-5 text-gray-600 ${
                   loadingStates.refreshing ? 'animate-spin' : ''
                 }`} />
               </button>

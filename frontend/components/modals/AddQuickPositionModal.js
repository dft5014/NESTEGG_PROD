...
const AddQuickPositionModal = ({ isOpen, onClose, onPositionsSaved, seedPositions }) => {
  ...
  const handleKeyDown = (e, assetType, positionId, fieldIndex) => {
    if (!shortcutsEnabled) return; // only active when toggle is ON
  ...
  const toggleSelectRow = (type, id, checked) => {
    const key = `${type}-${id}`;
  ...
  const toggleSelectAllInSection = (assetType, rows, checked) => {
    setSelectedPositions(prev => {
      const next = new Set(prev);
      rows.forEach(p => {
        const k = `${assetType}-${p.id}`;
  ...
  const deleteSelected = () => {
    ...
    selectedPositions.forEach(k => {
      ...
      const id = rest.join('-'); // ids can contain '-'
      if (!removeMap.has(t)) removeMap.set(t, new Set());
      removeMap.get(t).add(id);
    });
    setPositions(prev => {
      const out = { ...prev };
      removeMap.forEach((ids, t) => {
        out[t] = (out[t] || []).filter(p => !ids.has(String(p.id)));
      });
      return out;
    });
                       ...
                       return (
                        ...
                        >
                            <td className="px-3 py-2">
                              <input
                                type="checkbox"
                                checked={selectedPositions.has(`${assetType}-${position.id}`)}
                                onChange={(e) => toggleSelectRow(assetType, position.id, e.target.checked)}
                                className="w-4 h-4 rounded border-gray-300 text-blue-600"
                           ...
                           {config.fields.map(field => (
                            ...
                            <td className="px-2 py-2">
                              <div className="flex items-center justify-center space-x-2">
                                {/* status badge */}
                                {(() => {
                                  const key = `${assetType}-${position.id}`;
                                  const hasErrors = position.errors && Object.values(position.errors).some(Boolean);
                                  const importing = importState.queued.some(j => j.position?.id === position.id) || importState.inFlight > 0 && selectedPositions.has(key);
                                  const imported = position.status === 'success';
                                  if (imported) return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-green-50 text-green-700 border border-green-200"><CheckCircle className="w-3 h-3" />Imported</span>;
                                  if (importing) return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-amber-50 text-amber-700 border border-amber-200"><Loader2 className="w-3 h-3 animate-spin" />Sending</span>;
                                  if (hasErrors) return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-rose-50 text-rose-700 border border-rose-200"><AlertCircle className="w-3 h-3" />Fix</span>;
                                  return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-blue-50 text-blue-700 border border-blue-200"><Info className="w-3 h-3" />Ready</span>;
                                })()}
         ...
         <div className="flex items-center justify-between mb-4">
           ...
           <div className="flex items-center space-x-3">
            ...
            <button
              ...
              disabled={selectedPositions.size === 0 || importState.isRunning}
              className={`
                px-3 py-2 text-sm font-medium rounded-lg border
                ${selectedPositions.size === 0 || importState.isRunning
                  ? 'bg-gray-100 text-gray-400 cursor-not-allowed border-gray-200'
                  : 'bg-white text-gray-700 hover:bg-gray-50 border-gray-300'}
              `}
              title="Delete selected rows"
            >
              Delete Selected ({selectedPositions.size || 0})
            </button>

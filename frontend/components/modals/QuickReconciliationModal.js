// QuickReconciliationModal.js - Fully Fixed & Improved Version
...
// Memoized logo lookup for performance
...
// ===== Shared Currency Input Component (improved focus/selection) =====
export const CurrencyInput = React.memo(function CurrencyInput({
  ...
  return (
    <input
      ref={inputRef}
      id={ariaLabel?.replace(/\\s+/g, '_').toLowerCase()} // Use stable ID for focus
      type="text"
      inputMode="decimal"
      ...
      onFocus={(e) => {
        setFocused(true);
        onFocus?.(e);
        // Fix: Set cursor at the end to prevent overwriting
        requestAnimationFrame(() => {
          try {
...
export default function QuickReconciliationModal({ isOpen, onClose }) {
  ...
  }, [isOpen, userId, accounts, rawPositions, groupedLiabilities, refreshAccounts, refreshPositions, refreshLiabilities]);
  ...
  function WelcomeScreen() {
    ...
    }, [refreshPositions, refreshAccounts, refreshLiabilities]);
